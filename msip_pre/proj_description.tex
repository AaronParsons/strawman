...
%SF: Alternate version of next sentence (I find the current construction awkward): During these epochs, roughly 0.3$--$1~Gyr after the Big Bang, the first stars and black holes heated and reionized the Universe following cosmic recombination.
These epochs
correspond to
...
%SF: Another alternate version (trying to avoid too many "of"s in a row): Direct observation of the large scale structure of reionization, and its evolution with time, via the...
...
%SF: I would use "existing" instead of "current"
current
...
spectrum. 
%SF: I feel like, since we are "breaking" the roadmap by doing more than originally promised for the second generation experiment, maybe we should specify "According to this initial roadmap" or something here?
...
observing in the 
%SF: Add space before MHz?
50--225MHz band.
%SF: passive voice here - I would say "will deliver" instead of "is scoped to deliver"
...
%SF: Why not just say "and directly image the IGM"?  
...
%SF: possibly replace last sentence in this paragraph with an addition here: ", and the exploration of these early structures and their effects on each other and their environments was one of the top three ``priority science objectives chosen by the [NWNH] survey committee for the decade 2012-2021.""
.  During the Dark Ages
%SF: Italicize z in both parentheses
...
decade 2012-2021."
% XXX check decadal: make list or drop IGM
...
polarization 
%SF: I thought that Planck has not released any polarization info?  I believe their tau measurement was purely from the shape of the TT power spectrum
...
%SF: the panels aren't labeled a and b, switch to left-hand panel?
...
%SF: Should delete comma after reionization
...
%SF: I would say again here which stage of HERA will accomplish each of these specific goals
...
%SF: I don't think the connection between "the growth and intersection of ionization fronts" and Figure 2 is very clear, unless you are already an expert.  Nowhere in the text do we describe how a power spectrum relates to the underlying physics.  Maybe just say explicitly:  "As shown in Figure~\ref{fig:eor_pspec}, HERA will make precision measurements of the spatial power spectrum of neutral gas during the EoR, which ultimately depends upon the nature and distribution of the first stars and galaxies as they drive a complex network of ionization fronts through the IGM."
...
%SF: we do not define "IGM" anywhere.
...
of reionization 
%SF: I don't think that any of these are important references; if you are looking to describe the fundamental physics of reionization.  The semi-numeric models don't introduce any new ideas, they just implement them.  I would have chosen:
%http://adsabs.harvard.edu/abs/1987ApJ...321L.107S (first cosmological HII regions)
%http://adsabs.harvard.edu/abs/1997ApJ...483...21H (introducing the standard way to calculate x_HI(z) )
%http://adsabs.harvard.edu/abs/2004ApJ...613....1F (model of H II region sizes)
...
%SF: This last question seems to specific to me.  I would say: "What were the most important feedback mechanisms in the transition from the first stars to first galaxies, and how did they affect these populations?"
...
that are needed to 
%SF: I would use "advance" instead of "progress"
...
%SF: We do not define "cMpc" anywhere (I don't think it's used anywhere else either)
...
%SF: Is the next clause necessary?
...
%SF: I think comoving is preferred
...
%SF: I would say "shown in"
in Figure \ref{fig:eor_pspec}.

%SF: Alternate version of next sentence: "Recently, the MWA and PAPER teams have made substantial advances in understanding how smooth-spectrum foreground emission interacts with the instrument to produce the EoR Window."
...
%SF: Maybe add here: , in comparison to the $\sim 20$~mK$^2$ signal
(8 in mK$^2$;
...
%SF: "advances doesn't make sense to me here" - tools instead?
...
%SF: I don't think the next sentence works (either grammatically or in making sense).  Maybe try: "Foregrounds are bright within a ``wedge" (lower right) and then fall precipitously in the ``EoR window" (blue/black region), where measurements are thermal noise-limited.  The wedge's location can be estimated analytically (solid black line), but the foregrounds leak out by a margin dictated by the chromatic properties of both the foregrounds and the instrument."
...
%SF: We don't use "uv" anywhere else in the proposal, is it okay to introduce it now?  Should it be italicized?
complete uv coverage to $\sim$700~m for foreground imaging.
HERA draws on the technical heritage of the MWA, PAPER,
%SF: We don't ever explain what these experiments are.  I barely even know what MITEoR is.  Can we at least reference papers to define them?
...
%SF: You actually list two advantages here (not "the" advantage)!  I'd say: "This approach offers two advantages, providing early..."
...
%SF: As written, "is" doesn't seem right here.  Maybe better: "The timeline of HERA development and its associated science products is outlined below." 
...
\vspace{-7pt}
  ...
  %SF: Last one makes it sound like we don't know if we can do this.  I'd just say "Optimization of analysis algorithms to extract full sensitivity of the instrument, including partially coherent baselines.  Image large reionization structures."
...
\vspace{-8pt}
\noindent
%SF: I don't like the repetition of "stage" here.  Maybe \emph{HERA's gradual buildout will enable cutting edge science at each stage while mitigating project risk.}
...
%SF: It seems strange to call out the public data products in bold but not the other efforts, and also to mention the other efforts in the header but not the public data.  I would make them consistent.  Also, as written here it is not clear whether the training and diversity efforts are the same thing.  (Reading more, I think they are not.)  Maybe: "The HERA program will train new instrumentalists at both the graduate and undergraduate levels, increase the diversity of US graduate programs by engaging South African students and preparing them for admission to US degree programs, and make our major data products available publicly."
...
%SF: Can we list how many here (if it's an impressive number)?  Also, why say "project plan" and not just "project"?
...
%SF: Typo: should be "commissioning"
...
%SF: This last part strikes me as unnecessarily negative.  Why put the idea that our efforts may not pay off into the reviewers' heads?  I think it's sort of implicit.
should they
choose to do so.
...
HERA. 
%SF: Suggested replacement: "For example, HERA will provide the large-scale reionization environment for pointed JWST or ALMA galaxy observations [not sure Gong is the right reference here; there really isn't anybody who's focused on this, so I'd leave one out] and can offer cross-correlations with both WFIRST near-IR surveys [could cite http://adsabs.harvard.edu/abs/2009ApJ...690..252L] and CMB polarization observations citep{tashiro_et_al2010}."
...
Project Manager. 
%SF: Consistency note...nobody else gets a first name in this section, except Goeke
...
and analysis 
%SF: Again, I would replace "advances" with "tools"
...
%SF: "current thermal noise level"
...
%SF: Also shouldn't be a comma here
...
%SF: existing in place of current?
current
...
%SF: I would get rid of (ref: NWNH).  And I think it's okay to say "nearly all"
major astronomical facilities over the next decade (ref: NWNH).
Such studies include direct 
%SF: "direct observations"
observation of stars, gas, dust, and AGN in the
%SF: I don't see any reason to include CCAT here.  It is simply nowhere near the same class as the other instruments mentioned.  
...
%SF: perhaps more important than context: "providing the \emph{only} direct window onto the impact of these sources on their large-scale environments."
