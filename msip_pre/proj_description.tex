...
first-generation HERA instruments --- PAPER and the MWA ---
we propose a staged
build-out of HERA
...
constraints on $x_{\rm \HI}$ during reionization (colored symbols), along with a
reionization history consistent with these measurements (black line). The
constraints HERA would provide are overlaid with black markers (with the prior
that  $x_\HI$ steadily decreases over this redshift range). At redshifts 8--12
...
% XXX and from Carilli: burying the major science results from HERA in the
% project hardware schedule is a disaster. burying them in the middle of some
% paragraph somewhere is also a lost cause for busy referees.  these science
% headlines need to be absolutely clear and up front and stated more than once.
% i like them where they are., but end of section 1 would ok too.
...
% XXX from Morales: The bullets in the scientific justification need to be
% rewritten to reflect the figures (We're using 568 sensitivity, not 331,
% etc.). 
...
HERA (solid) relative to a fiducial ionization model (dashed line at $x_{\rm\HI}=0.47$ in both panels).  
In addition
...
\citealt{lidz_et_al2008} as a function of neutral fraction $x_{\rm\HI}$ (and redshift).
The spectrum initially follows the dark matter fluctuations at the upper edge
of the plot, falls as the densest regions reionize at $x_{\rm\HI}=0.8$ (inside-out in
this model), rises and flattens as galaxies ionize large bubbles in the IGM
($0.6<x_{\rm\HI}<0.2$), and finally
falls again as reionization completes at the bottom of the
plot. Contours indicate the predicted significance of HERA~568 observations
...
as observed using PAPER \citep{pober_et_al2013}. Bright foreground emission is
...
The HERA element (right) has been optimized for spectral smoothness and
